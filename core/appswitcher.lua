...
function match_grabber(rawkey, mod, _key)
...
local appswitcher = {clients_list = {}, index = 1, hotkeys = {}, keys = {}}
local cache = {}
...
function appswitcher:init()
    local style = {
        ...
        title_font      = "Roboto Condensed Bold 12",
        hotkeys         = { "1", "2", "3", "4", "5", "6", "7", "8", "9", "0" },
        font            = { font = "Sans", size = 16, face = 0, slant = 0 },
        color           = { border = beautiful.color.bg, text = "#aaaaaa", main = "#b1222b", preview_bg = "#b1222b80",
                            wibox  = "#202020", icon = "#a0a0a0", bg   = beautiful.color.black , gray = "#575757" },
        shape           = nil
    }
    ...
    self.keygrabber = function(mod, key, event)
        ...
        for _, k in ipairs(self.keys.all) do
            if match_grabber(k, mod, key) then k[3](); return false end
        end
    ...
    function widg.draw(_, _, cr, _, height)
        ...
        for i = 1, #self.clients_list do
            ...
            local ext = cr:text_extents(txt)
            cr:move_to(psize.width/2 - (ext.width/2 + ext.x_bearing) , psize.height + style.label_height)
            cr:show_text(txt)
    ...
    --------------------------------------------------------------------------------
    self.widget = widg
    self.titlebox = wibox.widget.textbox("APP SWITCHER")
...
function appswitcher:show(args)
    ...
    -- get client list
    local clients = util.clients.getAll()   
    if #clients == 0 then 
        return
    elseif #clients  == 1 then
        clients[1].minimized = false
        clients[1]:raise()
        return
    end
    ...
    -- center wibox
    self.wibox.x = (width - self.wibox.width) / 2
    self.wibox.y = (height - self.wibox.height) / 2
